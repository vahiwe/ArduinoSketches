...
void setup() {
  ...
  if (oldstarttime == -1) {
    ...
    cdiffstate = (cdiffpercent * range) / 100L;
    ndiffstate = (ndiffpercent * range) / 100L;
...
void loop() {
  ...
  cdiffpercent = (int)((cdiffstate * 100L) / range);
  ndiffpercent = (int)((ndiffstate * 100L) / range);
  ontimepercent = (int)((diffstate * 100L) / range);
  offtimepercent = 100 - ontimepercent;
  ...
  ThingSpeak.setField(OnTimeStatus, ontimepercent);
